...
기간이 없어 이 검사를 받지 않고, 종료일을 받지 못한 Event(`end_date`가 `NULL`)는
상한만 없습니다(시작일 이전은 여전히 거절).

항목의 가용성 판정 기준은 두 경로가 같습니다. 예전에는 약속 생성만 저장된
`event.status`를 봤는데, 그 값은 적재 시점 스냅샷이라 운영 기간이 남은 Event에도 약속을
만들 수 없는 경우가 있었습니다. 지금은 양쪽 모두 기준일과 운영 기간으로 판정합니다
(위 [공통 정책](#공통-정책) 참고).
...
참여 요청 본문은 **약속을 넣을 여정 `tripId` 하나**입니다. 방문 날짜는 받지
않습니다 — 약속이 이미 활동 날짜를 갖고 있어 참여자가 고를 여지가 없고, 고르는
것은 "그 날짜를 어느 여정에 넣을지"뿐입니다. 값이 없으면 `COMMON-001`입니다.

```json
{ "tripId": 7 }
```

`tripId`는 본인 여정이어야 하고(`JOURNEY-002`), 약속의 활동 날짜가 그 여정 기간
안이어야 합니다(`JOURNEY-007`). 방장이 생성 때 여정을 고르는 것과 같은 이유입니다
— 멤버십의 `trip_id`가 비면 [내가 참여 중인 약속 조회](#내가-참여-중인-약속-조회)에서
빠지고 QR 공동 소비 결제도 트립 비용 연결을 만들지 못합니다.

참여가 성사되면 같은 트랜잭션에서 참여자의 여정에도 항목(`trip_items`)을
`CONFIRMED`로 겁니다. 참여자가 **이미 `Add to journey`로 같은 장소를 같은 날짜에
담아 둔 경우**에는 새 행을 만들지 않고 그 행을 약속 항목으로 올립니다
(`appointment_id`·`confirmed_at`을 채우고 `CONFIRMED`로). `(trip_id, item_id,
visit_date)`가 살아 있는 행에 대해 UNIQUE라 새 행을 넣을 수 없는데, 담아 뒀다는
이유로 참여가 막히면 앞뒤가 맞지 않기 때문입니다. 그 자리에 **다른** 약속이 이미
걸려 있으면 `JOURNEY-004`로 거절합니다.

참여 성공 시 보증금 예치와 `ACTIVE` 전환이 같은 트랜잭션에서 함께 끝나므로,
방장이 아닌 회원의 참여 취소는 실질적으로 `ACTIVE` 상태에서 일어납니다.
취소 결과는 시각에 따라 갈립니다.

참여를 취소하면 그 약속으로 잡아 둔 여정 항목도 같은 트랜잭션에서 함께
내립니다(soft delete). 남겨 두면 참여하지도 않는 약속이 여정에 남습니다. 담아
두기만 했던 항목을 승격시킨 경우도 함께 사라집니다 — 승격 전 상태를 따로 기억하지
않기 때문입니다.
