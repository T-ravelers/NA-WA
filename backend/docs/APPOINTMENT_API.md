...
- 약속 생성·참여는 회원 지갑 차감, 시스템 에스크로(`DEPOSIT_POOL`) 입금, 양쪽
  ...
  전환합니다.
- 출석 확정이 성공하면 `DepositPayoutBatch`를 `PENDING`으로 생성합니다. 실제
  환급·노쇼 분배 지갑 이체는 아직 없는 별도 비동기 처리가 맡습니다(구현되면
  이 배치를 가져가 처리).
...
- 방장만 `IN_PROGRESS` 상태에서 모든 `ACTIVE` 회원을 정확히 한 번씩 `ATTENDED`
  ...
  `IN_PROGRESS`가 아니거나 요청이 활성 회원 전원을 정확히 한 번씩 포함하지
  않으면 `APPOINTMENT-006`을 반환합니다.
- 성공하면 같은 트랜잭션에서 각 회원의 출석 상태를 반영하고 약속을 `COMPLETED`로
  전환한 뒤, 보증금 정산 배치(`DepositPayoutBatch`)를 `PENDING`으로 생성합니다.
  이 시점에는 아직 지갑 이체가 일어나지 않습니다 — 실제 환급·노쇼 분배는 별도
  비동기 처리(아직 미구현)가 이 배치를 가져가 실행합니다.
