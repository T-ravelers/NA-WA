...
- 보증금은 5,000 이상 50,000 이하의 정수 포인트 금액입니다. 화면 단위는
  포인트(P)이고 지갑 원장은 `KRW`라 1포인트 = 1원으로 동일한 값을 씁니다 —
  전송 값·검증 범위·DB 제약은 원 단위 시절과 같습니다.
...
- **`IN_PROGRESS`인 것만으로는 부족합니다.** 이 상태는 활동 **시작** 시각에
  ...
  값을 읽지 못하면 `APPOINTMENT-009`를 반환합니다. 화면도 같은 조건으로 진입을
  막지만 화면에만 있는 조건은 화면을 거치지 않는 요청을 막지 못합니다.
