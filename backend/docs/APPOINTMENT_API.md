...
- 결제 API가 연결되기 전까지 생성 확인을 결제 완료로 간주합니다. 따라서 약속은
  생성 직후 `RECRUITING`, 방장은 `ACTIVE`, 방장의 보증금은 `HELD`로 기록되어
  목록과 상세에서 즉시 확인할 수 있습니다.
- 일반 참여 요청도 확인 시 결제 완료로 간주합니다. 참가자의 보증금은 `HELD`,
  참가자 상태는 `ACTIVE`로 기록하며, 실제 결제 연동 후에는 이 임시 예치 경로를
  결제 성공 콜백으로 대체합니다.
- `LEFT`가 된 회원은 같은 약속에 다시 참여할 수 없습니다.

## 약속 상태 전이

약속 상태는 방장의 시작 버튼으로 변경하지 않고, 주기 작업 또는 전이 API가 동일한
조건을 평가해 변경합니다.

- `RECRUITING` → `CLOSED`: 참여 마감 시각이 지났거나 `PENDING + ACTIVE` 예약 인원이
  정원에 도달한 경우
- `CLOSED` → `CONFIRMED`: `PENDING` 미납 회원은 `LEFT`/보증금 `CANCELLED`로 정리한 뒤,
  남은 모든 `ACTIVE` 회원의 보증금이 `HELD`인 경우
- `CONFIRMED` → `IN_PROGRESS`: 활동 시작 시각이 도달한 경우
- `IN_PROGRESS` → `COMPLETED`: 방장이 모든 `ACTIVE` 회원의 출석을 확정한 경우

수동·운영 확인이 필요한 경우 `POST /api/v1/appointments/{appointmentId}/status/transition`
을 호출할 수 있습니다. 이 API는 임의의 상태를 지정하지 않고 위 조건에 해당하는
전이만 수행합니다.
