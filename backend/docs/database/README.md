...
이 문서는 Flyway V9 적용 후의 NA-WA 데이터베이스를 도메인 단위로 보여줍니다.
전체 구조를 먼저 이해한 뒤 필요한 상세 ERD로 이동할 수 있습니다.

V9에는 서비스 테이블 41개가 있습니다. Flyway로 적용하면 관리용
...
DDL의 최종 기준은 `backend/src/main/resources/db/migration`의 V1~V9입니다. V7은
Event·Place의 출처·상세·미디어 필드를 추가하고 사용하지 않는 번역 테이블을 제거합니다.
V8은 KRW 통화와 회원 지갑을 채우는 데이터 시드라 구조를 바꾸지 않습니다. V9는 영수증
입력·항목 배분과 게임형 정산의 동의·결과 상태 테이블을 추가합니다.
이 문서는 V9 적용 DB의 `information_schema`에서 테이블과 외래키를 대조해 갱신해야 합니다.
