...
- 삭제되지 않은 일정 기준으로 같은 여정에는 동일한 탐색 항목과 방문 날짜 조합을
  한 번만 저장하며, soft delete 후 새 일정으로 다시 추가할 수 있습니다.
- 삭제되지 않은 일정 기준으로 같은 여정과 약속 조합을 한 번만 연결하며,
  soft delete 후 새 일정으로 다시 추가할 수 있습니다.
