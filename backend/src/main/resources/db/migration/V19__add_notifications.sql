...
--
-- created_at의 기본값은 다른 표와 맞춰 두지만, 알림을 적는 코드는 이 기본값에 기대지 않고
-- 시각을 직접 넣는다. 이 값이 화면에 그대로 보이고 목록 정렬 기준이라, DB와 앱의 시간대가
-- 어긋나면 알림 시각이 그만큼 통째로 밀리기 때문이다. 기본값은 다른 경로로 들어온 행을
-- 위한 안전망으로만 남긴다.
