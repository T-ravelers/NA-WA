...
for i in $(seq 1 10); do
  ...
  code=$(curl -s -o /dev/null -w '%{http_code}' --resolve api.na-wa.cloud:443:127.0.0.1 https://api.na-wa.cloud/ || true)
...
acme_code=$(curl -s -o /dev/null -w '%{http_code}' -H 'Host: api.na-wa.cloud' http://127.0.0.1/.well-known/acme-challenge/deploy-check || true)
