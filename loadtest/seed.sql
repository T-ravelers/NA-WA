...
-- 회차 사이의 ID 간격. 최대 VU(8,920)보다 커야 회차끼리 번호가 겹치지 않는다.
SET @run_stride = 10000;

-- 주 사용자 초기 잔액. 시드가 @runs개의 보증금을 미리 HELD로 잡으므로 그만큼 여유를
-- 둔다. 실행 중에는 충전(+50,000)이 소모(보증금 5,000 + QR 9,000)보다 커서 잔액이
-- 오히려 늘어난다 — 실측으로 확인했다.
SET @main_initial_balance = 200000.0000 + 10000.0000 * @runs;
...
SELECT wallet_owner_id, 'KRW',
       CASE
           WHEN member_id BETWEEN 900001 AND 900000 + @vu_count THEN @main_initial_balance
           -- 정산 참여자는 회차마다 약 4,500을 낸다. 회차에 맞춰 여유를 둔다.
           WHEN member_id BETWEEN 970001 AND 970000 + @vu_count
               THEN 50000.0000 + 20000.0000 * @runs
...
-- 실행 회차마다 새 약속을 쓴다.
--
-- 한 번 참여한 VU는 다시 참여할 수 없고(ALREADY_JOINED), 출석을 확정한 약속은
-- COMPLETED가 되어 두 번째 확정을 받지 않는다. 회차분을 미리 깔아 두면 볼륨을
-- 초기화하지 않고도 @runs 번까지 그대로 다시 돌릴 수 있다.
--
-- k6는 RUN_INDEX로 자기 회차를 고른다. 두 값이 어긋나면 없는 약속을 불러 404가
-- 나므로 README의 대응표를 함께 본다.
SET @recruiting_base = 1000000;
SET @hosted_base = 2000000;
...
WITH RECURSIVE
    run_seq(r) AS (SELECT 1 UNION ALL SELECT r + 1 FROM run_seq WHERE r < @runs),
    seq(n) AS (SELECT 1 UNION ALL SELECT n + 1 FROM seq WHERE n < @recruiting_count)
SELECT @recruiting_base + (r - 1) * @run_stride + n, 500000, 990000, 'en',
       CONCAT('loadtest-recruiting-r', r, '-', n),
       'Synthetic appointment for scenario 1',
...
WITH RECURSIVE
    run_seq(r) AS (SELECT 1 UNION ALL SELECT r + 1 FROM run_seq WHERE r < @runs),
    seq(n) AS (SELECT 1 UNION ALL SELECT n + 1 FROM seq WHERE n < @recruiting_count)
SELECT @recruiting_base + (r - 1) * @run_stride + n, 990000, NULL, 'ACTIVE', 'PENDING'
FROM run_seq CROSS JOIN seq;
...
-- 확정하면 COMPLETED가 되어 재사용할 수 없으므로 회차분을 함께 깐다.
INSERT INTO appointments (
    appointment_id, item_id, host_member_id, language_code, appointment_name,
    appointment_description, max_members, deposit_amount, appointment_status,
    meeting_place, activity_start_at, activity_end_at
...
WITH RECURSIVE
    run_seq(r) AS (SELECT 1 UNION ALL SELECT r + 1 FROM run_seq WHERE r < @runs),
    seq(n) AS (SELECT 1 UNION ALL SELECT n + 1 FROM seq WHERE n < @vu_count)
SELECT @hosted_base + (r - 1) * @run_stride + n, 500000, 900000 + n, 'en',
       CONCAT('loadtest-hosted-r', r, '-', n),
       'Synthetic completed-activity appointment',
       2, 5000.0000, 'IN_PROGRESS', 'Load-test venue',
...
WITH RECURSIVE
    run_seq(r) AS (SELECT 1 UNION ALL SELECT r + 1 FROM run_seq WHERE r < @runs),
    seq(n) AS (SELECT 1 UNION ALL SELECT n + 1 FROM seq WHERE n < @vu_count)
SELECT @hosted_base + (r - 1) * @run_stride + n, 900000 + n, NULL, 'ACTIVE', 'PENDING'
FROM run_seq CROSS JOIN seq;

-- 출석 확정은 HELD 보증금을 요구한다. 회원 지갑 -> DEPOSIT_POOL 이체와
-- 원장을 시드에도 같이 남겨 런타임 정합성을 유지한다.
-- 회차마다 하나씩 잡아 두므로 회원 한 명이 @runs개의 HELD 보증금을 갖는다.
...
WITH RECURSIVE
    run_seq(r) AS (SELECT 1 UNION ALL SELECT r + 1 FROM run_seq WHERE r < @runs),
    seq(n) AS (SELECT 1 UNION ALL SELECT n + 1 FROM seq WHERE n < @vu_count)
SELECT 'KRW', 900000 + n, CONCAT('SEED-HOSTED-DEPOSIT-', r, '-', n), 'DEPOSIT_HOLD',
       'COMPLETED', 5000.0000, CONCAT('seed-hosted-deposit-', r, '-', n),
       'Load-test hosted appointment deposit', CURRENT_TIMESTAMP
FROM run_seq CROSS JOIN seq;

-- balance_after 는 회원별로 순차 차감된 값이어야 한다. 회차분을 한꺼번에 넣으므로
-- 같은 회원의 몇 번째 차감인지를 세어 계산한다.
INSERT INTO wallet_ledger_entries (
    transfer_id, wallet_id, entry_type, amount, balance_after
)
SELECT wt.transfer_id, w.wallet_id, 'DEBIT', 5000.0000,
       @main_initial_balance - 5000.0000 * ROW_NUMBER() OVER (
           PARTITION BY wt.initiator_member_id ORDER BY wt.transfer_id
       )
...
SET w.available_balance = @main_initial_balance - 5000.0000 * @runs
...
SET pool.available_balance = 5000.0000 * @vu_count * @runs
WHERE pool_owner.owner_type = 'SYSTEM' AND pool_owner.system_code = 'DEPOSIT_POOL';

-- 회차별 약속 하나에 그 회차의 이체 하나를 짝지운다. 약속 번호에서 회차와 VU를
-- 되계산해 이체의 멱등키와 맞춘다.
...
JOIN wallet_transfers wt
 ...
 AND wt.idempotency_key = CONCAT(
         'seed-hosted-deposit-',
         FLOOR((am.appointment_id - @hosted_base) / @run_stride) + 1,
         '-',
         am.member_id - 900000)
WHERE am.appointment_id >= @hosted_base
  AND am.appointment_id < @hosted_base + @run_stride * @runs
  AND MOD(am.appointment_id - @hosted_base, @run_stride) = am.member_id - 900000
...
WHERE appointment_id >= @recruiting_base
  AND appointment_id < @recruiting_base + @run_stride * @runs
UNION ALL
...
WHERE appointment_id >= @hosted_base
  AND appointment_id < @hosted_base + @run_stride * @runs
UNION ALL
