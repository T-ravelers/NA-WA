...
`AWS_*` 값은 정산 영수증 이미지와 향후 프로필 사진을 저장하는 S3에 쓰입니다.
버킷은 백엔드가 도는 계정과 **다른 AWS 계정**이 소유하므로 EC2 인스턴스 역할이
아니라 버킷 소유 계정이 발급한 IAM 사용자 키로 접근합니다. `AWS_S3_AVATAR_BUCKET`은
프로필 사진 첨부(#444)를 위한 예약 변수이며 현재 애플리케이션은 아직 읽지 않습니다.
저장 계약은 [아바타 S3 저장 계약](./backend/docs/AVATAR_STORAGE.md)에 있습니다.
`AWS_ACCESS_KEY_ID`와 `AWS_SECRET_ACCESS_KEY`는 항상 **한 쌍**입니다. 둘 다
비워두면 AWS SDK가 다른 경로에서 자격증명을 찾으므로, S3 기능을 쓰지 않는
...
보이다가 S3를 처음 사용할 때 실패하고, 원인이 "키 하나 누락"이라는 것을
찾기 어렵습니다.
