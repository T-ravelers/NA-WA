...
## 화면 조형 — 규격을 직접 쓰지 않습니다

화면 컨테이너와 헤더는 **[개발 컨벤션의 「화면 조형 맞추기」](./docs/DEVELOPMENT_CONVENTION.md#화면-조형-맞추기)**
가 정본입니다. 이웃 파일을 베끼지 말고 그것을 보세요.

> 2026-08-23~24 실측에서 뒤로가기 버튼이 5종, 화면 제목이 3종, 컨테이너 위 여백이
> `pt-6`·`pt-8`·`py-8`·`pt-14`·조건부 또는 없음의 다섯 갈래였고, `ExploreView`는 하단 탭
> 여백을 `AppShell`과 겹쳐 208px을 뒀습니다(#489).
