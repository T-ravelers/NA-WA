...
/**
 * Event와 Place 필터가 공유하는 operational_v9 taxonomy다.
 * Sector ID는 1~4, Activity ID는 1~56으로 고정한다.
 */
...
export const VALID_EXPLORE_SECTOR_IDS = new Set(EVENT_SECTOR_OPTIONS.map((sector) => sector.id))
export const VALID_EXPLORE_ACTIVITY_IDS = new Set(
  EVENT_ACTIVITY_OPTIONS.map((activity) => activity.id),
)
