...
/**
 * Shared Event and Place taxonomy from the operational_v9 handoff.
 *
 * Sector IDs 1..4 and Activity IDs 1..56 are shared by both item types.
 * Keep these IDs aligned with the local taxonomy SQL and replace this
 * constant with the taxonomy API response when that contract is available.
 */
export const EXPLORE_SECTOR_OPTIONS: ExploreSectorOption[] = [
  {
    id: 1,
...
// Keep the existing Event names as compatibility aliases for current callers.
export const EVENT_SECTOR_OPTIONS = EXPLORE_SECTOR_OPTIONS
export const EVENT_ACTIVITY_OPTIONS = EXPLORE_SECTOR_OPTIONS.flatMap((sector) => sector.activities)
export const PLACE_SECTOR_OPTIONS = EXPLORE_SECTOR_OPTIONS
