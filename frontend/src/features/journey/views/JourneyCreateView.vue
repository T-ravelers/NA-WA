...
// 히스토리가 없을 때(딥링크·PWA 재진입)만 쓰는 뒤로 가기 목적지. 제출 후 복귀와
// 같은 규칙이다 — 다른 화면이 보낸 경우 그 화면으로 나머지 query(itemId·itemType
// 등)를 그대로 들고 돌아가고, 직접 들어온 경우는 여정 목록이다. 만든 것이
// 없으니 tripId는 더하지 않는다.
const backFallback = computed<RouteLocationRaw>(() => {
...
/**
 * 뒤로 가면 왔던 길을 되감는다. 목적지를 push하면 이 화면이 히스토리에 남아서,
 * 돌아간 약속 생성 화면에서 시트를 닫을 때(그쪽도 되감기) 목록이 아니라 이
 * 화면으로 다시 튄다. 되감으면 이 화면 항목이 빠져 그 문제가 없다.
 */
function goBack(): void {
  ...
  void router.push(backFallback.value)
}
