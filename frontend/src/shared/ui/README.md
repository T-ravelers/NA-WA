...
| `ScreenHeader`                               | **화면 제목 헤더.** `back`/`root` 두 조형. 직접 `<header>`를 쓰지 않는다 |
...
6. **한 줄 제목과 버튼 라벨은 `shared/lib/fitText`의 `v-fit-text`로 줄인다.** 칸보다 길면
   말줄임 대신 글자 크기를 하한(제목 50% · 버튼 75%)까지 줄이고, `.wrap`을 주면 하한에서
   줄을 꺾는다. `AppButton` 라벨에는 이미 걸려 있으니 화면에서 다시 걸지 않는다. 여러 줄로
   꺾는 제목과 목록 카드 제목에는 쓰지 않는다 — 거기는 한 줄 말줄임이 맞다.
   **나란히 놓여 크기가 같아야 하는 줄**은 컨테이너에 `v-fit-text-group`을 건다. 그 안의
   요소가 가장 많이 줄여야 하는 쪽의 비율을 함께 쓴다 — 짧은 라벨도 따라 작아진다.
7. 컴포넌트가 없어서 막히면 만들지 말고 프론트엔드 리드에게 말한다.
...
- `on-paper` (#171717) — `paper` / `paper-fill` 면 위 (종이톤 티켓 스텁, primary 버튼)
- `on-category` (#171717) — 소비영역 코어색 면 위 (채워진 칩, 카테고리 티켓)
