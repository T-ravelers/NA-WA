 ...
 * 다시 적지 않는다.
 *
 * **눌림 반응(`active:scale-[0.98]`)은 세그먼트에도 붙는다 — 의도한 변화다.** 원래
 * `SegmentedControl`에는 없었고 칩 두 곳에만 있었다. 같은 컨트롤인데 누를 때 하나는
 * 반응하고 하나는 가만히 있는 것이 갈라진 상태였으므로 여기서 맞춘다. 정적 스냅샷은
 * `:active`를 찍지 못하니 픽셀 대조로는 드러나지 않는다(#305 리뷰).
 */
