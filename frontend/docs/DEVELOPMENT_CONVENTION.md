...
- **이 왕복은 히스토리에 한 자리만 차지합니다.** 같은 화면이 두 번 쌓이면 돌아온 뒤
  ...
  뒤라면 빈 화면으로 열립니다. 그래서 **보내는 쪽과 받는 쪽 중 한쪽만 자리를 지킵니다.**
  - **돌려줄 결과 값이 없으면 보내는 쪽이 자리를 지킵니다**(약속 생성 ↔ 충전). 떠나기
    전에 자기 엔트리를 돌아올 상태로 `replace` 해 두고(`resume=1`) 목적지를 `push`
    합니다. 받는 쪽은 일을 마치든 그냥 나가든 **`router.back()`으로 자기 엔트리를
    소비합니다** — 바로 아래가 이미 목적지입니다.
  - **돌려줄 결과 값이 있으면 받는 쪽이 자리를 넘겨받습니다**(약속 생성 ↔ 여정 생성의
    새 `tripId`). 보내는 쪽이 **`replace`로 자리를 내주고**, 받는 쪽이 일을 마치든 그냥
    나가든 **`router.replace`로 그 자리를 돌려줍니다.** 나머지 query는 건드리지 않고
    돌려주되 자기 화면의 키(`amount` 등)만 빼고 결과 키(`tripId`)를 더합니다.
  - 되감거나 돌려줄 히스토리가 없을 때(딥링크·PWA 재진입)만 `returnRouteName` 또는 자기
    목록으로 보냅니다. 받는 쪽의 뒤로 가기 버튼도 같은 규칙입니다.
- **제출이 끝난 화면은 자리를 내줍니다**(`replace`). 호출자가 없을 때도 마찬가지입니다 —
  여정 생성이 제출 후 상세로 `push`하면, 상세에서 뒤로 갈 때 방금 제출한 폼이 다시 뜹니다.
- 떠나는 동안 사라질 입력(폼 초안)을 이어 가야 하면 보내는 쪽이 `sessionStorage`에 저장하고,
  **떠나기 전에 자기 엔트리를 `resume=1`로 `replace`** 해 둡니다. 브라우저 뒤로가기는 떠날
  때의 URL 그대로 되돌리므로, 표시를 남겨야 받는 쪽 버튼으로 오든 뒤로가기로 오든 같은
  자리에서 되살아납니다. 되살린 뒤에는 **즉시 지웁니다.** 저장하는 것은 폼 초안뿐이며,
  어느 항목의 초안인지(`itemId`·`tripId` 등)를 함께 적어 다른 진입에서는 무시합니다.
  인증 토큰은 저장하지 않습니다.
...
| `Explore`                              | `Discover`            | 탐색 화면의 이름입니다(`explore.title`). 시안 V2는 화면 헤더를 `DISCOVER`로, 하단 탭을 `Explore`로 그려 시안 안에서 두 말이 갈립니다. 라우트(`/explore`)와 i18n 키(`explore.*`)가 이미 `explore`이므로 화면 문구도 `Explore`로 맞춥니다 |
