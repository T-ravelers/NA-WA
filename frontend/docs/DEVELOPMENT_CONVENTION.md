...
- 받는 쪽은 일을 마치면 `returnRouteName`으로 **`router.replace`** 합니다. 나머지 query는
  건드리지 않고 돌려주되, 자기 화면의 키(`amount` 등)만 빼고 결과 키(`tripId`,
  `resume=1` 등)를 더합니다. `push`하면 제출이 끝난 화면이 히스토리에 남아, 돌아간
  화면에서 뒤로 가기(되감기)를 누를 때 그 화면이 다시 튀어나옵니다.
- 받는 쪽의 뒤로 가기 버튼은 목적지를 `push`하지 않고 **`router.back()`으로 되감습니다.**
  히스토리가 없을 때(딥링크·PWA 재진입)만 `returnRouteName` 또는 자기 목록으로 보냅니다.
- 떠나는 동안 사라질 입력(폼 초안)을 이어 가야 하면 보내는 쪽이 `sessionStorage`에 저장하고,
  받는 쪽이 `resume=1`을 붙여 돌려보내면 되살린 뒤 **즉시 지웁니다.** 저장하는 것은 폼
  초안뿐이며, 어느 항목의 초안인지(`itemId`·`tripId` 등)를 함께 적어 다른 진입에서는
  무시합니다. 인증 토큰은 저장하지 않습니다.
