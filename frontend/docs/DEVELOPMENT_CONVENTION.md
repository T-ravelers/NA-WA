...
- **받는 쪽은 돌아갈 때 자기 히스토리 엔트리를 소비합니다.** 일을 마쳤든 그냥 나가든
  끝난 화면이 히스토리에 남으면, 돌아간 화면에서 뒤로 가기를 누를 때 그 화면이 다시
  튀어나옵니다. 돌아가는 방법은 넘길 값이 있는지로 갈립니다.
  - **결과 값을 query로 실어야 하면**(여정 생성의 `tripId`) `returnRouteName`으로
    **`router.replace`** 합니다. 나머지 query는 건드리지 않고 돌려주되 자기 화면의
    키(`amount` 등)만 뺍니다. 이때 보내는 쪽은 자기 엔트리를 미리 바꾸지 않습니다.
  - **넘길 값이 없고 보내는 쪽이 돌아올 자리를 미리 표시해 뒀으면**(아래 `resume=1`)
    **`router.back()`으로 되감습니다.** 바로 아래 엔트리가 이미 목적지라, 여기서 목적지를
    새로 만들어 `replace`하면 같은 화면이 히스토리에 두 번 쌓입니다. 그러면 돌아간
    화면에서 뒤로 가기를 눌러도 흐름을 벗어나지 못하고 같은 라우트에 머물며, 그 자리는
    초안을 이미 지운 뒤라 빈 화면으로 열립니다.
  - 되감을 히스토리가 없을 때(딥링크·PWA 재진입)만 `returnRouteName` 또는 자기 목록으로
    보냅니다. 받는 쪽의 뒤로 가기 버튼도 같은 규칙입니다.
